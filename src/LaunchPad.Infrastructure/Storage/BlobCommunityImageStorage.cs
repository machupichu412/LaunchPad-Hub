...
/// <summary>
/// Real implementation — writes into the same existing "artifacts" container
/// BlobProfilePictureStorage uses, under a community/{postId}/{guid}.{ext} prefix instead of
/// avatars/{appUserId}/..., via managed identity. Only registered when Storage:AccountUrl is
/// configured (see Program.cs); this sandbox has no real Azure Storage account, so
/// LocalDiskCommunityImageStorage is what actually runs here.
/// </summary>
public sealed class BlobCommunityImageStorage : ICommunityImageStorage
{
    private const string ContainerName = "artifacts";
    private readonly BlobContainerClient _container;

    public BlobCommunityImageStorage(IConfiguration configuration)
    {
    ...
    {
        var blobName = $"community/{postId}/{Guid.NewGuid()}{ExtensionFor(contentType)}";
